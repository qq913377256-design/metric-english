...
- V1.3首批已接入进阶阅读核心引擎：基础阅读、职场写作、进阶阅读三阶段切换，以及按完成顺序推进的60天状态机
- 进阶阅读每篇分为精读日和应用日；应用日包含40–80词英文业务回答、实时词数和自动保存
- V1.3已加入30篇原创进阶阅读，按SQL与建模、指标产品、实验统计、报告沟通、质量治理、策略表达六个模块组织
- 阅读资料库现有40篇文章，支持难度、模块、完成状态组合筛选；进阶阶段可单独重置，不影响基础阅读、写作和词汇记录
- 进阶训练记录按所属模块回看已保存的应用回答、字数和完成日期；未作答项目不占用记录页，并可重新打开文章比较参考版本
- 理解题答案在精读日完成后解锁，40–80词应用参考版本在应用日完成后解锁；训练页面不再混用资料库“已读”按钮
...
阅读问题部分可添加`### Answer Key`。V1.3进阶文章还必须添加`### Workplace Application`和`### Application Model Response`。应用日会把题面显示在英文回答编辑器上方；理解题答案和应用参考版本按训练进度分阶段解锁

## 浏览器数据

学习进度保存在当前浏览器的`metricEnglish.v1`键中，当前数据格式为schema 4。schema 1、2、3旧数据会自动迁移，并保留阅读完成、词汇、阅读位置和V1.2写作记录。schema 4新增`advancedReadingProgram`，按第1至60天保存任务、英文应用回答和完成时间。清除浏览器站点数据、换电脑或使用不同浏览器后，进度不会自动同步

换电脑前，在首页“学习信号”中点击“导出进度”。在另一台电脑打开网站后点击“导入进度”，选择导出的JSON文件并确认覆盖。导入不会上传文件，也不会自动合并两台电脑的记录

## 验证

项目包含`tests/browser-smoke.js`，覆盖schema 1、2、3到4迁移、三阶段隔离、正式30篇进阶内容、写作顺序推进、60天进阶阅读映射、跨日期不跳题、英文回答恢复、40词门槛、分阶段答案解锁、进阶记录回看、保存失败回退、用户文本防注入、Markdown加载与重试、V1.1阅读回归、移动端抽屉和多视口溢出检查
