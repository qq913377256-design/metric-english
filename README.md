...
阅读问题部分可添加`### Answer Key`。V1.3进阶文章还必须添加`### Workplace Application`，应用日会把题面显示在英文回答编辑器上方。网站会把答案折叠，读者点击后查看
...
项目包含`tests/browser-smoke.js`，覆盖schema 1、2、3到4迁移、三阶段隔离、正式30篇进阶内容、写作顺序推进、60天进阶阅读映射、跨日期不跳题、英文回答恢复、40词门槛、保存失败回退、用户文本防注入、Markdown加载与重试、V1.1阅读回归、移动端抽屉和多视口溢出检查
